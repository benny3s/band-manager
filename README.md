...
| 관리 | 주로 Benny | 날짜 후보·멤버·시간 범위·제목·응답 초기화 |

링크는 **하나뿐**이다: `https://benny3s.github.io/band-manager/`
`#admin` 을 붙이면 열자마자 관리 탭으로 바로 들어간다(북마크용). 탭 자체는 누구에게나 보인다.

관리 탭을 숨기지 않는 이유: 저장소가 공개라 어차피 진짜 보안이 안 되고,
삭제·초기화는 모두 확인창이 뜨며, 사고가 나도 스프레드시트 버전 기록으로 되돌릴 수 있다.
